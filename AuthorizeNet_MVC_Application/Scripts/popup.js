...
(function () {
	...
	AuthorizeNetPopup.closePopup = function() {
		...
		document.getElementById("iframeAuthorizeNet").src = "/Parker/empty";
	...
	function openSpecificPopup(opt) {
        ...
        //Commented for fixing QA bug 212.
		//var form; 
		//if (navigator.appVersion.indexOf(".NET CLR") >= 0) {
		//    var formss = document.forms["formAuthorizeNetPopup"];
		//    form = formss.formAuthorizeNetPopup[0];
		//}
		//else {
		//    form = document.forms["formAuthorizeNetPopup"];
		//}

		switch (opt.action) {
			case "addPayment":
				ifrm.style.width = "453px";
				ifrm.style.height = AuthorizeNetPopup.options.eCheckEnabled ? "508px" : "513px";
				break;
			case "editPayment":
		...
		if (form.elements["PaymentProfileId"]) {
		    form.elements["PaymentProfileId"].value = opt.paymentProfileId ? opt.paymentProfileId : "";
		}

		if (form.elements["ShippingAddressId"]) {
		    form.elements["ShippingAddressId"].value = opt.shippingAddressId ? opt.shippingAddressId : "";
		}
